(function (doc, win) {
  var docEl = doc.documentElement,
    ...
    recalc = function () {
      ...
      window.rem = docEl.style.fontSize.substring(
        0,
        docEl.style.fontSize.length - 2
      );
    };
...
function renderAppContainer(apps) {
  ...
  for (let i = 0; i < apps.length; i++) {
    ...
    appDownload.setAttribute("class", "download");
    appDownload.setAttribute(
      "onclick",
      "window.open('./apps/" + apps[i].name + ".jar')"
    );
